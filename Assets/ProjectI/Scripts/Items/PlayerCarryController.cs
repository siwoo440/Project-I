...
namespace ProjectI.Items // 아이템 기능 네임스페이스
{
    ...
    public sealed class PlayerCarryController : MonoBehaviour // CarryPoint 종속 방식의 한손·양손 아이템 운반
    {
        [SerializeField] private Transform viewTransform; // 시선과 투척 방향 기준
        ...
        [SerializeField] private float throwVelocityChange = 7f; // 투척 순간 속도 변화량
        [SerializeField] private LayerMask obstructionMask = ~0; // 내려놓기·투척 공간 검사 레이어
        private PlayerInputReader inputReader; // 플레이어 입력 래퍼
        private WorldItem heldItem; // 현재 직접 운반 중인 아이템

        public bool HasItem => heldItem != null; // 현재 아이템 운반 여부 공개
        public WorldItem HeldItem => heldItem; // 현재 운반 아이템 공개
        ...
        private void Update() // 프레임별 내려놓기·투척 입력 처리
        {
            if (heldItem == null) // 현재 아이템 존재 여부 확인
            {
                return; // 운반 입력 처리 중단
            }

            if (inputReader.DropPressed) // G 내려놓기 입력 확인
            {
                DropHeldItem(); // 현재 아이템 내려놓기
                return; // 같은 프레임 투척 방지
            }

            if (inputReader.ThrowPressed) // 마우스 왼쪽 투척 입력 확인
            {
                ThrowHeldItem(); // 현재 아이템 앞으로 투척
            }
        }

        private void LateUpdate() // PlayerLook의 카메라 회전 적용 뒤 아이템을 정확히 재동기화
        {
        ...
        public void Configure(Transform view, Transform oneHandPoint, Transform twoHandPoint, PlayerInputReader reader) // 에디터 자동 설정용 참조 지정
        {
        ...
        public bool TryPickup(WorldItem item) // 월드 아이템 직접 운반 시작
        {
            if (item == null || heldItem != null || item.IsHeld || viewTransform == null) // 아이템 누락·손 점유·시점 누락 여부 확인
            {
                return false; // 획득 실패 반환
            }

            ResolveCarryPoints(); // 최신 운반 지점 연결 상태 확인
            Transform carryPoint = GetCarryPoint(item.CarryType); // 아이템 운반 방식에 맞는 한손·양손 지점 선택

            if (carryPoint == null) // 필요한 운반 지점 누락 확인
            {
                return false; // 운반 포즈를 만들 수 없으면 획득 실패 반환
            }

            heldItem = item; // 현재 운반 아이템 저장
            heldItem.IgnoreCollisionsWith(transform); // 플레이어와 아이템이 서로 부딪히지 않고 겹칠 수 있도록 충돌 무시
            heldItem.BeginCarry(carryPoint); // CarryPoint 자식으로 직접 연결하여 즉시 운반 시작
            return true; // 획득 성공 반환
        }

        public void DropHeldItem() // 현재 운반 아이템을 플레이어 앞에 내려놓기
        {
            ...
            Vector3 releasePosition = CalculateReleasePosition(dropDistance); // 플레이어 앞 안전한 내려놓기 위치 계산
            Quaternion releaseRotation = Quaternion.Euler(0f, viewTransform.eulerAngles.y, 0f); // 바닥 배치를 위한 수평 회전 계산
            WorldItem itemToRelease = heldItem; // 해제 전 로컬 참조 저장
            heldItem = null; // 플레이어 운반 상태 비우기
            itemToRelease.Release(releasePosition, releaseRotation, Vector3.zero); // 부모 연결 해제와 함께 월드 물리 복구
        }

        public void ThrowHeldItem() // 현재 운반 아이템 전방 투척
        {
            if (heldItem == null || viewTransform == null) // 운반 아이템 또는 시점 누락 확인
            {
                return; // 투척 중단
            ...
            Vector3 releasePosition = CalculateReleasePosition(Mathf.Max(0.85f, dropDistance)); // 플레이어와 겹치지 않는 투척 시작 위치 계산
            ...
            WorldItem itemToRelease = heldItem; // 해제 전 로컬 참조 저장
            heldItem = null; // 플레이어 운반 상태 비우기
            itemToRelease.Release(releasePosition, releaseRotation, velocityChange); // 부모 연결 해제 후 물리 복구와 동시에 투척
        }
        ...
        private Vector3 CalculateReleasePosition(float distance) // 플레이어 바로 앞의 낮은 위치 계산
        {
            ...
            {
                if (ShouldIgnoreHit(hit.collider)) // 플레이어 또는 현재 아이템인지 확인
                {
                    continue; // 자기 자신은 배치 장애물에서 제외
                }

                safeDistance = Mathf.Min(safeDistance, Mathf.Max(0.20f, hit.distance - obstructionPadding)); // 장애물이 있으면 바로 앞 안전 위치로 제한
            }

            return origin + (flatForward * safeDistance); // 플레이어 발앞 가까운 위치에서 물리적으로 떨어지도록 반환
        }
        ...
        private void OnValidate() // 인스펙터 값 검증
        {
            ...
            throwVelocityChange = Mathf.Max(0.1f, throwVelocityChange); // 투척 속도 최소값 보정
        }
    }
}
