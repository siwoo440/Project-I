...
namespace ProjectI.Combat // 공통 전투 시스템 네임스페이스
{
    public sealed class DamageSource : MonoBehaviour // 함정·환경·몬스터 공격이 재사용할 범용 피해 발생기
    {
        ...
        [SerializeField] private float staggerPower = 0f; // 기본 경직 누적 힘
        ...
        public float StaggerPower => staggerPower; // 기본 경직 힘 공개

        public void Configure(CombatFaction faction, CombatDamageType type, float damage, float forceAmount, GameObject sourceInstigator) // 기존 Day14 자동 구성 호환 설정
        {
            Configure(faction, type, damage, 0f, forceAmount, sourceInstigator); // 기존 피해 발생기는 경직 힘 0으로 새 설정에 연결
        }

        public void Configure(CombatFaction faction, CombatDamageType type, float damage, float targetStaggerPower, float forceAmount, GameObject sourceInstigator) // 경직·넉백을 포함한 범용 피해 발생기 설정
        {
            sourceFaction = faction; // 피해 발생기 진영 저장
            damageType = type; // 피해 종류 저장
            baseDamage = Mathf.Max(0f, damage); // 기본 피해량 음수 방지
            staggerPower = Mathf.Max(0f, targetStaggerPower); // 경직 힘 음수 방지
        ...
        public bool TryDamage(Collider targetCollider, Vector3 hitPoint, Vector3 hitNormal, Vector3 forceDirection, int attackId, out CombatHitResult result) // 지정 Collider에 공통 피해 적용 시도
        {
            ...
            DamageInfo damageInfo = new DamageInfo(gameObject, instigator, sourceFaction, damageType, baseDamage, hitPoint, hitNormal, staggerPower, appliedForce, attackId); // 경직·넉백을 포함한 공통 피해 요청 생성
