...
namespace ProjectI.Combat // 공통 전투 시스템 네임스페이스
{
    ...
    public sealed class AttackDefinition : ScriptableObject // 검·도끼·맨손·몬스터 공격이 공유하는 공격 데이터
    {
        ...
        [SerializeField] private float staggerPower = 10f; // 향후 경직 계산용 공격 힘
        [SerializeField] private float knockbackForce = 1.5f; // 향후 넉백 계산용 힘
        ...
        public float ActiveDuration => activeDuration; // 판정 활성 시간 공개
        ...
        public float StaggerPower => staggerPower; // 향후 경직 힘 공개
        public float KnockbackForce => knockbackForce; // 향후 넉백 힘 공개

        public void Configure(string targetDisplayName, float damage, CombatDamageType type, float stamina, float windup, float active, float recovery, float movement, float radius, float stagger, float knockback) // Day14 테스트 공격 데이터 자동 구성
        {
