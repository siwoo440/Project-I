...
namespace ProjectI.Persistence // 일차 저장·복구 네임스페이스
{
    public sealed class DailySnapshotStore // 로컬 Current와 불변 일차 스냅샷 파일 저장소
    {
        ...
        private readonly string currentPath; // 현재 사무소 안전 체크포인트 파일 경로
        private readonly string dailyFolder; // 불변 완료 일차 스냅샷 폴더
        ...
        public DailySnapshotStore() // 기본 persistentDataPath 저장소 생성
        {
            ...
            currentPath = Path.Combine(rootPath, "Current", "current.json"); // 현재 사무소 안전 체크포인트 경로 설정
        ...
        public bool WriteCurrent(DailySnapshotData data) // 현재 사무소 안전 체크포인트를 교체 저장
        {
            if (data == null || !IsOfficeSnapshot(data)) // 저장 데이터와 안전 구역 여부 확인
            {
                return false; // 던전 진행 상태를 Current로 저장하지 않음
            }
        ...
        public bool WriteImmutableDailySnapshot(DailySnapshotData data) // 완료 일차 사무소 스냅샷을 한 번만 저장
        {
            if (data == null || data.completedDay <= 0 || !IsOfficeSnapshot(data)) // 완료 일차와 안전 구역 데이터인지 확인
            {
                return false; // 잘못된 일차 또는 던전 상태 저장 차단
            }

            Directory.CreateDirectory(dailyFolder); // 일차 백업 폴더 보장
            string path = GetDailyPath(data.completedDay); // 해당 완료 일차 파일 경로 계산

            if (File.Exists(path)) // 이전 시도에서 같은 일차 파일이 이미 생성됐는지 확인
            {
                if (TryReadPath(path, out DailySnapshotData existing, out _) && existing.completedDay == data.completedDay && IsOfficeSnapshot(existing)) // 기존 파일이 정상 완료 백업인지 확인
                {
                    return true; // 정상 기존 백업은 재시도 가능한 성공 상태로 취급
                }

                return false; // 손상되거나 의미가 다른 기존 파일은 임의 덮어쓰지 않음
            }
        ...
        public bool TryReadDailySnapshot(int completedDay, out DailySnapshotData data, out string reason) // 특정 완료 일차 백업을 검증하여 읽기
        {
            string path = GetDailyPath(completedDay); // 요청한 일차 파일 경로 계산
            return TryReadPath(path, out data, out reason); // SHA-256 검증을 포함해 읽기
        }

        public bool TryReadLatestValidDailySnapshot(out DailySnapshotData data, out string path) // 가장 최근 정상 사무소 일차 스냅샷 검색
        {
            data = null; // 기본 결과 초기화
            ...
            foreach (string candidate in files) // 최신 파일부터 순회
            {
                ...
                if (candidateData.completedDay <= 0 || !IsOfficeSnapshot(candidateData)) // 던전 중간 저장이나 잘못된 완료 일차인지 확인
                {
                    continue; // 안전한 사무소 체크포인트만 복구 후보로 사용
                }
        ...
        public static bool IsOfficeSnapshot(DailySnapshotData data) // 복구 기준으로 사용할 수 있는 사무소 스냅샷인지 확인
        {
            return data != null && string.Equals(data.activeDestination, "Office", StringComparison.OrdinalIgnoreCase); // Office만 안전 체크포인트로 인정
        }
        ...
        private bool TryReadPath(string path, out DailySnapshotData data, out string reason) // 파일 단위 검증·역직렬화 공통 처리
        {
            ...
            try // 손상 파일 예외를 복구 흐름으로 전환
            {
                ...
                parsed.items ??= new System.Collections.Generic.List<ProjectI.Items.ItemInstanceData>(); // 빈 목록 안전 보정
                parsed.economy ??= new EconomySnapshotData(); // 경제 데이터 누락 안전 보정
