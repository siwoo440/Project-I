...
namespace ProjectI.EditorTools // 에디터 도구 네임스페이스
{
    public static class Phase16Day37NetworkSetup // 협동 네트워크 프리팹 구성 (37일차)
    {
        ...
        [MenuItem("Project I/Day 39/Rebuild Network Prefabs (Combat Sync)")] // 39일차: 전투·장치 동기화 포함 다시 만들기
        public static void RebuildForDay39() // 같은 구성 (월드 상태에 전투 동기화 포함)
        {
            Build(); // 구성
        }
        ...
        public static void Build() // 구성
        {
            ...
            report.AppendLine($"월드 상태 {WorldStatePath} · 고유 번호 {HashOf(worldState)} · 아이템 동기화 {(worldState.GetComponent<NetItemSync>() != null ? "포함" : "없음")} · 전투 동기화 {(worldState.GetComponent<NetCombatSync>() != null ? "포함" : "없음")}"); // 월드
        ...
        private static GameObject BuildWorldState() // 방장 기준 월드 상태
        {
            ...
            root.AddComponent<NetCombatSync>(); // 39일차 몬스터·피해·함정·장치 동기화
