...
namespace ProjectI // 프로젝트 공통 네임스페이스
{
    public class GameSettingsUI : MonoBehaviour // 메인 메뉴 게임 설정 화면 관리
    {
        ...
        void ApplyAndClose() // 편집 중인 설정 저장과 적용 후 화면 닫기
        {
            ...
            settingsManager.SaveAndApply(draftSensitivity, draftVolume, draftFullScreen, draftVSync, resolution.x, resolution.y); // 편집한 설정 저장과 적용
            windowOpen = false; // 설정 화면 닫기
        }

        void OnGUI() // OnGUI 기반 게임 설정 화면 표시
        {
            ...
            float width = 650f; // 설정 창 너비
            float height = 620f; // 설정 창 높이
            ...
            labelStyle.fontSize = 17; // 설정 항목 글자 크기 설정
            labelStyle.alignment = TextAnchor.MiddleLeft; // 설정 항목 왼쪽 정렬

            GUIStyle valueStyle = new GUIStyle(GUI.skin.label); // 설정값 표시용 GUI 스타일 생성
            valueStyle.fontSize = 17; // 설정값 글자 크기 설정
            valueStyle.alignment = TextAnchor.MiddleRight; // 설정값 오른쪽 정렬

            GUI.Box(new Rect(0f, 0f, Screen.width, Screen.height), string.Empty); // 메뉴 뒤쪽 입력을 차단할 전체 화면 배경 표시
            GUI.Box(new Rect(x, y, width, height), string.Empty); // 게임 설정 창 배경 표시
            GUI.Label(new Rect(x + 20f, y + 25f, width - 40f, 55f), "게임 설정", titleStyle); // 게임 설정 제목 표시

            GUI.Label(new Rect(x + 55f, y + 105f, 230f, 30f), "마우스 감도", labelStyle); // 마우스 감도 항목 표시
            GUI.Label(new Rect(x + 440f, y + 105f, 140f, 30f), $"{draftSensitivity:F2}배", valueStyle); // 현재 마우스 감도 표시
            draftSensitivity = GUI.HorizontalSlider(new Rect(x + 55f, y + 145f, 525f, 25f), draftSensitivity, 0.25f, 3f); // 마우스 감도 조절 슬라이더 표시

            GUI.Label(new Rect(x + 55f, y + 190f, 230f, 30f), "전체 음량", labelStyle); // 전체 음량 항목 표시
            GUI.Label(new Rect(x + 440f, y + 190f, 140f, 30f), $"{draftVolume * 100f:F0}%", valueStyle); // 현재 전체 음량 표시
            draftVolume = GUI.HorizontalSlider(new Rect(x + 55f, y + 230f, 525f, 25f), draftVolume, 0f, 1f); // 전체 음량 조절 슬라이더 표시

            GUI.Label(new Rect(x + 55f, y + 280f, 180f, 35f), "해상도", labelStyle); // 해상도 항목 표시

            if (GUI.Button(new Rect(x + 255f, y + 280f, 50f, 35f), "<")) // 이전 해상도 버튼 입력 확인
            {
                SelectPreviousResolution(); // 이전 해상도 선택
            }

            GUI.Label(new Rect(x + 315f, y + 280f, 180f, 35f), $"{selectedResolution.x} x {selectedResolution.y}", valueStyle); // 선택한 해상도 표시

            if (GUI.Button(new Rect(x + 515f, y + 280f, 50f, 35f), ">")) // 다음 해상도 버튼 입력 확인
            {
                SelectNextResolution(); // 다음 해상도 선택
            }

            draftFullScreen = GUI.Toggle(new Rect(x + 55f, y + 345f, 250f, 35f), draftFullScreen, " 전체 화면"); // 전체 화면 설정 토글 표시
            draftVSync = GUI.Toggle(new Rect(x + 355f, y + 345f, 250f, 35f), draftVSync, " 수직 동기화"); // 수직 동기화 설정 토글 표시

            GUI.Label(new Rect(x + 55f, y + 400f, width - 110f, 45f), "해상도와 전체 화면 설정은 적용 버튼을 누른 뒤 변경됩니다.", labelStyle); // 화면 설정 적용 안내 표시
            GUI.Label(new Rect(x + 55f, y + 445f, width - 110f, 45f), "전체 음량은 현재 모든 AudioSource에 동일하게 적용됩니다.", labelStyle); // 음량 적용 방식 안내 표시

            if (GUI.Button(new Rect(x + 45f, y + 520f, 170f, 50f), "적용 및 저장")) // 설정 적용 버튼 입력 확인
            {
                ApplyAndClose(); // 편집한 설정 저장과 적용
            }

            if (GUI.Button(new Rect(x + 240f, y + 520f, 170f, 50f), "기본값")) // 기본 설정 버튼 입력 확인
            {
                SetDraftDefaults(); // 편집값을 기본 설정으로 변경
            }

            if (GUI.Button(new Rect(x + 435f, y + 520f, 170f, 50f), "취소")) // 설정 취소 버튼 입력 확인
            {
