...
namespace ProjectI // 프로젝트 공통 네임스페이스
{
    public class CrackedWall : MonoBehaviour, IInteractable // 폭탄을 소모해 파괴하는 금간 벽
    {
        ...
        [SerializeField] GameObject wallVisual; // 파괴할 벽 외형과 Collider가 포함된 오브젝트
        [SerializeField] string requiredItemName = "폭탄"; // 벽 파괴에 필요한 아이템 이름

        bool isDestroyed; // 벽이 이미 파괴되었는지 저장

        public string GetPrompt() // 벽 상태에 맞는 상호작용 안내 반환
        {
            if (isDestroyed) // 벽이 이미 파괴되었는지 확인
            {
                return "파괴된 벽"; // 파괴 완료 안내 반환
            }
        ...
        {
            if (isDestroyed) // 이미 파괴된 벽인지 확인
            {
                return; // 중복 파괴 방지
            }

            if (interactor == null || interactor.Inventory == null) // 플레이어 인벤토리를 사용할 수 있는지 확인
            {
                return; // 벽 파괴 처리 중단
            }
            ...
            if (wallVisual != null) // 파괴할 벽 외형이 연결되어 있는지 확인
            {
                wallVisual.SetActive(false); // 벽 외형과 Collider 비활성화
            }
            else // 별도 벽 외형이 없는 경우
            {
                gameObject.SetActive(false); // 현재 금간 벽 오브젝트 전체 비활성화
            }
        }
    }
}