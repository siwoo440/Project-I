...
namespace ProjectI // 프로젝트 공통 네임스페이스
{
    ...
    {
        public static VillageShopManager Instance { get; private set; } // 현재 활성 상점 매니저 접근점
        ...
        void Awake() // 상점 매니저 싱글톤과 초기 재고 설정
        {
            if (Instance != null && Instance != this) // 기존 상점 매니저 존재 여부 확인
            {
                Destroy(gameObject); // Scene 전환으로 생성된 중복 매니저 제거
                return; // 중복 초기화 중단
            }

            Instance = this; // 현재 오브젝트를 전역 상점 매니저로 저장
            transform.SetParent(null); // DontDestroyOnLoad 적용을 위해 루트로 분리
            DontDestroyOnLoad(gameObject); // Scene 전환 후 구매품 목록 유지
        ...
        void OnDestroy() // 상점 매니저 싱글톤 참조 정리
        {
            if (Instance == this) // 현재 오브젝트가 등록된 상점 매니저인지 확인
            {
                Instance = null; // 전역 상점 매니저 참조 초기화
            }
        }
        ...
        public bool TryPurchase(int index) // 상점 아이템 하나 구매 시도
        {
            ...
            VillageShopStockEntry entry = GetStockEntry(index); // 구매할 재고 항목 가져오기
            CampaignManager campaignManager = CampaignManager.Instance; // 구매 비용을 지불할 캠페인 매니저 가져오기
